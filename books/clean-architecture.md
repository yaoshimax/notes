...
- 誤りの例
  ...
  - SOLIDの設計原則を使うと…
    ...
    - これならTaxiUIだけ変更すれば良い
      - 猫用の実装はFinder/Selecter/Dispatcherの派生クラスとして新規追加

#### 28章: テスト境界
- テストの種類はいろいろだが、アーキテクチャの観点からするとどれも同じ。アーキテクチャ円の最も外側で、独立してデプロイできるもの
  - もっとも、極端に分離してシステム設計の範囲外と捉えてはいけない
- システムと強く結合したテストは、少しかえるとたくさんのテストが壊れる（脆弱なテストの問題）
- 解決策は、変化しやすいものに依存ないようにすること。このため、テストから利用できるAPIを作り、すべてのビジネスルールを検証できるようすべき。
  - 全クラスのテストをするクラスと、全メソッドのテストメソッドを含んだテストスイートは、クラスやメソッドの変更で大量にテスト変更が必要（構造的結合）
  - テストAPIの役割は、アプリケーションの構造をテストから隠すこと
  - ただし、このようなAPIはシステムをテスト可能とするための強い権限をもつので、プロダクションコードにデプロイすると危険

#### 29章: クリーン組み込みアーキテクチャ
- ソフトウェアは消耗しないが、ファームウェアやハードウェアは時代遅れになる
- 管理できていないファームウェアやハードウェアの依存により、ソフトウェアが内部から破壊される可能性がある
- ファームウェアは少なくして、ソフトウェアを多くすべき。
  - コードにSQLを埋め込んだりするのは実質ファームウェア（ソフトウェアとハードウェアのやりとりをするもの）
- 組み込み開発ではしばしば、ターゲットハードウェアのボトルネックを重視して、そのターゲットでしかコードをテストできない状態になりがち。これは開発速度を低下させる
- ソフトウェアとファームウェアの境界はハードウェア抽象化レイヤー（HAL)と呼ばれる。ここはサービスを提供するもので、どのように機能するかをソフトウェアに明らかにすることはない。
  - フラッシュメモリにデータを格納するファームウェアは、フラッシュメモリの実装は隠すべき
  - GPIOに接続されたLEDも、Led_TurnOn(5)とかではなく、Indicate_LowBattery()みたいなインターフェースをHALは提供すべき
- ハードウェア詳細はHALユーザに明らかにしない
　- プロセッサは詳細
  　- 特定プロセッサ用のグローバル変数をベンダが提供してるC言語は、もはやC言語ではない。それは詳細である。
    - `typedef char Int_8;` みたいな型定義が記述されたプロセッサ提供のヘッダファイルは直接使わず、これを利用するヘッダファイルを一段かましてつかうべし
      - 別のプロセッサに移植するときに便利
  - OSも詳細
- `#ifdef BOARD_V2` みたいな条件付きコンパイルも多用するのではなく、HALを使って抽象化するのが良い

### 第六部: 詳細

#### 30章: データベースは詳細
- データベースは詳細なので、アーキテクチャの構成要素として現れることはない
- RDBを使ってるにしても、データが表形式であるという知識は下位レベルのユーティリティ関数に閉じ込めるべき。
- そもそもデータベースが流行ったのは、ディスクが遅いから。RAMに全データが格納できるなら、リンクリストとかスタックとかのデータ構造でアクセスする

#### 31章: ウェブは詳細
- 処理を一極集中させるか分散させるか、歴史的に行ったり来たりしてる
- 個人向けファイナンスアプリ（デスクトップGUI)がブラウザ風GUIに変更したけど不評で元に戻した話
  - アーキテクトとしては、ビジネスルールをGUIから切り話せれば良かった
- GUIは詳細である。ウェブはGUIである。したがってウェブは詳細である
- アプリケーションとGUIのやり取りは、GUIの種類に特化した方法で行われている（ドラッグ＆ドロップとか）。なのでこういった操作をIOデバイスのように抽象化するのは難しい。
- しかし、ビジネスロジックはユースケースの集まりであり、ここのユースケースは入力・処理・出力に基づいて記述できる。この境界は抽象化ができる。

#### 32章: フレームワークは詳細
- フレームワークはアーキテクチャではない
- フレームワークは概してシステムの内部に組み込んでもらおうとしがちだが、リスクもある
  - フレームワークのアーキテクチャに難があることが少なくない。
    - フレームワークのコードを継承してビジネスオブジェクトにするとかNG！
  - プロダクトの成長に応じてフレームワークの提供機能では手に負えなくなる
  - フレームワークの進化する方向が思ってたのと違う
- 対策：フレームワークはアーキテクチャの円の外側として扱う
  - フレームワークの基底クラスからビジネスオブジェクトを派生させることを求められても断る。代わりにプロキシを用意して、コアコードのプラグインとする
  - Springでビジネスオブジェクトに`@autowired` アノテーションを散りばめてはいけない。Mainコンポーネントに依存性を注入する（Mainは最下位コンポーネントなので依存してて良い）
- 避けられないものもある（C++のSTLとか）が、リスクがあることは認識するべし

#### 33章: 事例、動画販売サイト
- 動画販売ウェブサイト用ソフトウェア
- ユースケース分析
  - アクター：動画作者、管理者、購入者、視聴者
  - カタログの閲覧・ライセンスの購入は抽象的なユースケース。「視聴者として」「購入者として」という説明付きのものが具体なユースケース
- コンポーネントアーキテクチャの検討
  - アクターごとにview/presentater/interactor/controllerのコンポーネントが用意される形
  - デプロイは各コンポーネントごと(例えば動画作者用のview、で1単位)でもよいし、viewは全部まとめてでも良いし、view/presenterすべて+その他すべて、という2つのjarとしても良い。
    - ↑のような選択肢を選べる状態にしておくことが大事
- 依存性管理
  - 制御の流れだと`controller -> interactor -> presenter -> view` の順
  - 依存の流れだと`view -|> presenter -|> interactor <- controller` （interactorが上位レベルのコンポーネント。それに向かう方向に向いている）

#### 34章: 書き残したこと
- 悪魔は実装の詳細に宿る
- オンライン書店の構築を考える。顧客から注文ステータスを見られるようにするというユースケースを実装したい
- コードのまとめ方の例１：レイヤードアーキテクチャ
 - システムを水平に見て同じレベルのものをまとめる。以下のクラスからなる（インターフェースと実装は同居するので）3つのパッケージ
   - OrdersController: ウェブからのリクエストを処理する
   - OrdersService: ビジネスロジックを定義するインターフェース
   - OrdersServiceImpl: ↑の実装
   - OrdersRepository: 永続された注文情報へのアクセス方法定義インターフェース
   - JdbcOrdersRepository: ↑の実装
 - 動くものを複雑になりすぎないように手早く作るにはレイヤードアーキテクチャは優れた方法。
 - だが、問題もある
   - 規模が大きくなると3つの巨大なバケツに入れるだけでは手に負えない
   - ビジネスドメインが異なっても気味悪いほどアーキテクチャがそっくり
- コードのまとめ方の例２: 機能によるパッケージング
  - 垂直方向の切り分け。関連する機能・ドメインの概念・集約ルートに基づいて分割
  - クラスやインターフェースは先と一緒。パッケージは一つ
  - メリット
    - トップレベルのコードの構成がビジネスドメインを感じさせる
- コードのまとめ方の例３：ポートとアダプター
...
- コードのまとめ方の例４：コンポーネントによるパッケージング
  ...
  - 静的解析ツールでルール違反をチェックさせても良いが、別の選択肢としてコンポーネントごとにまとめるというものがある。
  - 粒度の粗いコンポーネントの単位でパッケージを切る
    - 今回の例で言えばOrdersController / それ以外に分ける
    - それ以外を１パッケージにまとめてるというのはつまり、ビジネスロジックと永続化コードをまとめてコンポーネントとしているということ。
